'use client';
import { useState } from "react";
import { useResume } from "@/context/ResumeContext";
import { FiMenu, FiPlus, FiTrash2, FiLink } from "react-icons/fi";
import { FaGripLinesVertical } from "react-icons/fa6";
import { DragDropContext, Droppable, Draggable, DropResult } from "@hello-pangea/dnd";
import { Popover, PopoverContent, PopoverTrigger } from "@/components/ui/popover";
import { Calendar } from "@/components/ui/calendar";
import { Button } from "@/components/ui/button";
import { format } from "date-fns";

type Volunteering = {
  organization: string;
  role: string;
  startDate?: string;
  endDate?: string;
  url?: string;
  description?: string;
};

export default function VolunteeringPanel() {
  const { resume, setResume } = useResume();
  const volunteering: Volunteering[] = resume.volunteering || [];
  const [showDialog, setShowDialog] = useState(false);
  const [newVol, setNewVol] = useState<{
    organization: string;
    role: string;
    startDate: Date | null;
    endDate: Date | null;
    url: string;
    description: string;
  }>({
    organization: "",
    role: "",
    startDate: null,
    endDate: null,
    url: "",
    description: "",
  });

  // Drag-and-drop reorder
  const onDragEnd = (result: DropResult) => {
    if (!result.destination) return;
    const reordered = Array.from(volunteering);
    const [removed] = reordered.splice(result.source.index, 1);
    reordered.splice(result.destination.index, 0, removed);
    setResume(prev => ({
      ...prev,
      volunteering: reordered,
    }));
  };

  const handleCreate = () => {
    if (!newVol.organization.trim() || !newVol.role.trim()) return;
    setResume(prev => ({
      ...prev,
      volunteering: [
        ...(prev.volunteering || []),
        {
          organization: newVol.organization.trim(),
          role: newVol.role.trim(),
          startDate: newVol.startDate ? newVol.startDate.toISOString().split("T")[0] : undefined,
          endDate: newVol.endDate ? newVol.endDate.toISOString().split("T")[0] : undefined,
          url: newVol.url.trim(),
          description: newVol.description.trim(),
        },
      ],
    }));
    setShowDialog(false);
    setNewVol({
      organization: "",
      role: "",
      startDate: null,
      endDate: null,
      url: "",
      description: "",
    });
  };

  const handleDelete = (idx: number) => {
    setResume(prev => ({
      ...prev,
      volunteering: prev.volunteering?.filter((_, i) => i !== idx),
    }));
  };

  return (
    <section className="w-82 mx-auto">
      <div className="flex items-center gap-2 mb-4">
        <span className="bg-[#232323] rounded-full p-2">
          <FiMenu size={22} className="text-white" />
        </span>
        <h2 className="text-3xl font-bold">Volunteering</h2>
        <div className="ml-auto">
          <FiMenu size={22} className="text-gray-400" />
        </div>
      </div>

      <DragDropContext onDragEnd={onDragEnd}>
        <Droppable droppableId="volunteering">
          {(provided) => (
            <div {...provided.droppableProps} ref={provided.innerRef} className="space-y-2 mb-4">
              {volunteering.map((vol, idx) => (
                <Draggable key={idx} draggableId={String(idx)} index={idx}>
                  {(provided) => (
                    <div
                      ref={provided.innerRef}
                      {...provided.draggableProps}
                      className="bg-[#181818] border border-[#222] rounded p-4 flex items-center mb-2"
                    >
                      <span {...provided.dragHandleProps} className="mr-3 cursor-grab">
                        <FaGripLinesVertical className="text-gray-500" />
                      </span>
                      <div className="flex-1">
                        <div className="font-bold text-white">{vol.organization}</div>
                        <div className="text-gray-400 text-sm">{vol.role}</div>
                        <div className="text-xs text-gray-500">
                          {vol.startDate && vol.endDate
                            ? `${vol.startDate} - ${vol.endDate}`
                            : vol.startDate
                            ? `${vol.startDate} - Present`
                            : ""}
                        </div>
                        {vol.description && <div className="text-xs text-gray-300 mt-1">{vol.description}</div>}
                        {vol.url && (
                          <a
                            href={vol.url}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="text-xs text-blue-400 hover:underline flex items-center gap-1 mt-1"
                          >
                            <FiLink /> {vol.url}
                          </a>
                        )}
                      </div>
                      <button
                        className="ml-2 text-red-400 hover:text-red-600"
                        onClick={() => handleDelete(idx)}
                        type="button"
                        title="Delete"
                      >
                        <FiTrash2 />
                      </button>
                    </div>
                  )}
                </Draggable>
              ))}
              {provided.placeholder}
            </div>
          )}
        </Droppable>
      </DragDropContext>

      {showDialog ? (
        <div className="bg-[#181818] border border-[#222] rounded p-6 mb-4">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-bold">Add Volunteering</h3>
            <button onClick={() => setShowDialog(false)} className="text-gray-400 hover:text-white text-xl">&times;</button>
          </div>
          <input
            className="w-full p-2 rounded bg-[#181818] border border-[#333] text-white mb-2"
            value={newVol.organization}
            onChange={e => setNewVol(v => ({ ...v, organization: e.target.value }))}
            placeholder="Organization"
          />
          <input
            className="w-full p-2 rounded bg-[#181818] border border-[#333] text-white mb-2"
            value={newVol.role}
            onChange={e => setNewVol(v => ({ ...v, role: e.target.value }))}
            placeholder="Role"
          />
          <div className="mb-2">
            <label className="block mb-1">Start Date</label>
            <Popover>
              <PopoverTrigger asChild>
                <Button
                  variant="outline"
                  className="w-full justify-start text-left font-normal bg-[#181818] border border-[#333] text-white"
                >
                  {newVol.startDate ? format(newVol.startDate, "PPP") : "Select Start Date"}
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-auto p-0 bg-[#181818] border border-[#333]">
                <Calendar
                  mode="single"
                  selected={newVol.startDate ?? undefined}
                  onSelect={date => setNewVol(v => ({ ...v, startDate: date ?? null }))}
                  initialFocus
                />
              </PopoverContent>
            </Popover>
          </div>
          <div className="mb-2">
            <label className="block mb-1">End Date</label>
            <Popover>
              <PopoverTrigger asChild>
                <Button
                  variant="outline"
                  className="w-full justify-start text-left font-normal bg-[#181818] border border-[#333] text-white"
                >
                  {newVol.endDate ? format(newVol.endDate, "PPP") : "Select End Date"}
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-auto p-0 bg-[#181818] border border-[#333]">
                <Calendar
                  mode="single"
                  selected={newVol.endDate ?? undefined}
                  onSelect={date => setNewVol(v => ({ ...v, endDate: date ?? null }))}
                  initialFocus
                />
              </PopoverContent>
            </Popover>
          </div>
          <input
            className="w-full p-2 rounded bg-[#181818] border border-[#333] text-white mb-2"
            value={newVol.url}
            onChange={e => setNewVol(v => ({ ...v, url: e.target.value }))}
            placeholder="https://organization.org"
          />
          <textarea
            className="w-full p-2 rounded bg-[#181818] border border-[#333] text-white mb-2"
            value={newVol.description}
            onChange={e => setNewVol(v => ({ ...v, description: e.target.value }))}
            placeholder="Description (optional)"
          />
          <button
            className="mt-2 bg-white text-black px-4 py-2 rounded"
            onClick={handleCreate}
          >
            Add
          </button>
        </div>
      ) : (
        <button
          className="flex items-center gap-2 border border-[#222] text-white px-4 py-2 rounded hover:bg-[#181818] transition w-full justify-center"
          onClick={() => setShowDialog(true)}
          type="button"
        >
          <FiPlus />
          Add a new item
        </button>
      )}
    </section>
  );
}